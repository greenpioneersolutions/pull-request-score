 ...
 */
export function writeOutput(metrics: OutputMetrics, opts: WriteOutputOptions = {}): void {
  ...
  if (format === 'csv') {
    const rows = [
      ['metric', 'median', 'p95'],
      ['cycleTime', String(metrics.cycleTime.median ?? ''), String(metrics.cycleTime.p95 ?? '')],
      ['pickupTime', String(metrics.pickupTime.median ?? ''), String(metrics.pickupTime.p95 ?? '')],
    ];
